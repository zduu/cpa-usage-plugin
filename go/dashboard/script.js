...
function renderApiDetailContent(apiData, detailState) {
  ...
  const models = detail ? (detail.model_stats || []).map((m) => ({ name: m.model || 'unknown', requests: num(m.total_requests), success: num(m.success_count), failure: num(m.failure_count), tokens: num(m.total_tokens), total_tokens: num(m.total_tokens), input_tokens: num(m.input_tokens), output_tokens: num(m.output_tokens), cached_tokens: num(m.cached_tokens), cache_write_tokens: num(m.cache_write_tokens), reasoning_tokens: num(m.reasoning_tokens), providers: m.providers || [], avgLatency: num(m.avg_latency_ms), cost: aggregateCost(m, modelPrices, manualModelPrices) })) : Object.entries(apiData.models || {}).map(([name, m]) => ({ name, requests: num(m.total_requests), success: num(m.success_count), failure: num(m.failure_count), tokens: num(m.total_tokens), total_tokens: num(m.total_tokens), input_tokens: num(m.input_tokens), output_tokens: num(m.output_tokens), cached_tokens: num(m.cached_tokens), cache_write_tokens: num(m.cache_write_tokens), reasoning_tokens: num(m.reasoning_tokens), providers: m.providers || [], avgLatency: num(m.avg_latency_ms), cost: aggregateCost(Object.assign({ model: name }, m), modelPrices, manualModelPrices) }));
  ...
  // The server should expose estimated_cost consistently on both the summary
  // and model_stats; otherwise this total can differ from locally priced bars.
  const totalCost = Object.prototype.hasOwnProperty.call(summary, 'estimated_cost') ? Math.max(num(summary.estimated_cost), 0) : models.reduce((s, m) => s + m.cost, 0);
