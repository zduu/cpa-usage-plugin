...
- 修复看板摘要、兼容数据和上游接口详情接口偶发返回空 payload 时，前端读取 `generated_at` 或 `recent_events` 导致页面报错的问题。
- 上游接口详情遇到临时空响应时会显示可读的加载失败信息，并在后续轮询恢复后继续正常渲染。

### 验证

- 增加看板前端测试，覆盖 `dashboard-summary`、`dashboard-data` 和 `dashboard-api-detail` 返回空数据的兼容路径。
