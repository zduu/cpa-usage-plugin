...
当前代码版本：`2.6.4`；下一版本开发中，尚未发布。候选修复了匿名资源路由数据泄露和从 `v2.6.4` 升级时累计用量重复入账，并带来范围查询优化与完整用量备份；它不是"整体低资源"版本，SQLite 仍未接管运行时历史存储。变更、验证结果与已知边界见[更新日志](docs/releases/changelog.md)。
...
- 下一版本候选修复了匿名资源数据访问：只公开静态看板，数据接口统一走管理鉴权；旧资源别名不再提供数据。仍在使用 `v2.6.4` 及更早版本时的访问限制与迁移说明见[管理 API 安全边界](docs/guides/cpa-usage.md#6-管理-api-使用)。
...
- 手动模型价格支持最多 16 条不重叠的时段覆盖规则，按 `pricing_timezone` 和请求发生时间选择价格；旧价格 JSON 未包含 `time_rules` 时保持原有全天价格语义。受明细保留上限淘汰的记录会保留精简统计账本，保留窗口内的历史记录仍按请求时间应用时段价格。
