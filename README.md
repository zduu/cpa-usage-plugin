...
当前代码版本：`2.6.5`。本版本修复了匿名资源路由数据泄露和从 `v2.6.4` 升级时累计用量重复入账，并带来范围查询优化与完整用量备份；它不是"整体低资源"版本，SQLite 仍未接管运行时历史存储。变更、验证结果与已知边界见[更新日志](docs/releases/changelog.md)。
...
- `v2.6.5` 修复了匿名资源数据访问：只公开静态看板，数据接口统一走管理鉴权；旧资源别名不再提供数据。仍在使用 `v2.6.4` 及更早版本时的访问限制与迁移说明见[管理 API 安全边界](docs/guides/cpa-usage.md#6-管理-api-使用)。
