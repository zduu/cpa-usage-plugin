...
- 在 CPA 原生 usage 记录缺失时，可从成功响应体或流式 chunk 中的 usage 字段写入兜底统计，并通过延迟写入和指纹匹配避免与原生记录重复计数。兜底记录拿不到上游身份时会按客户端协议暂记为 `openai-compatible`，随后按模型、客户端 key、token 和完成时刻与任意上游家族的原生记录配对合并，不再把 Claude、Gemini 等上游的请求留成一条重复的 `openai-compatible` 记录。
