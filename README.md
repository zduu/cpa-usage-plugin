...
- 默认使用内存统计；可通过 `storage_enabled` + `storage_path` 开启后台队列 JSONL 持久化，配合周期 snapshot 和可选 fsync 在重启或更新插件后恢复保留窗口内的统计。
- 运行时元数据：页面可见当前保留策略、存储明细数、淘汰数、最近导入结果。
- 健康检查端点 `/health` 可查看插件运行状态、持久化状态、后台 writer 批次/滑动平均/压力指标和看板查询/缓存指标。
- 事件导出支持按筛选条件输出 JSON、CSV、JSONL，并可通过 `gzip=1` 压缩响应。
...
| `/dashboard-events-export` | GET | 按筛选条件导出全部匹配事件，默认 JSON；支持 `format=csv|jsonl` 和 `gzip=1`。 |
...
`/dashboard-summary`、`/dashboard-events`、`/dashboard-api-detail` 和 `/dashboard-events-export` 支持弱 ETag；内置看板轮询会自动使用 `If-None-Match`，外部脚本也可用条件请求减少未变化数据的重复传输。
