...
本版本在插件内部补齐旧宿主原生用量缺失的端点与流式标记，不需要修改或重编译 CPA。接口以未修改的 CPA v7.2.152 为准。

- 使用已有请求拦截与完成回调获取真实路径及流式状态；原生 usage 仍是唯一用量计数来源
- 保持响应拦截和流式分片拦截关闭，不恢复实时兜底，不修改请求或响应内容
- 并发候选冲突时保留未知值，不覆盖原生明确上报的字段；缓存有容量和时间上限
- 删除旧 v2.6.4 中要求修改 CPA 的补丁及说明
- 修复管理接口状态码的宿主 ABI 拼写兼容，确保 304 和错误状态能被 CPA 正确读取
- 回归覆盖 Responses、Chat Completions、Messages、Gemini、取消/失败请求、重试、并发歧义、原生字段优先、快照与 JSONL 恢复

旧宿主的 usage 不含请求 ID，无法对所有并发场景保证端点补齐；未提供请求回调或缺少必要信息时保持未知。具体边界见[部署文档](../guides/cpa-usage.md#cpa-版本兼容与统计兜底)。

## v2.6.3 - 2026-09-06

本版本关闭响应拦截实时兜底，改为依赖新版 CPA 原生用量上报，并修复统计明细保留路径的 CPU 与内存分配开销。

- 关闭 `response_interceptor` 与 `response_stream_interceptor` 能力，避免流式请求重复传输请求体、历史分片和维护延迟匹配状态
- 明细达到保留上限时按当前模型裁剪并清理底层引用，避免每次写入复制整个明细窗口
- 延后过期明细和 dedup 索引的全局扫描，仅在最早到期时间执行
- 保留旧版兜底数据导入、恢复及去重修复能力
- 通过 Go race、`go vet`、共享库构建和 Dashboard 前端测试
...
- 协议关联候选按各自原生请求时长动态复用完成时刻容差并增加 1 秒调度余量（最长保留 6 秒）；普通精确指纹仍使用原有 750ms 窗口，延长保留时间不会放宽最终配对条件
...
本版本修复模型统计中缓存命中率可能错误显示为 `0.0%` 的问题，并统一正常摘要与兼容回退模式的缓存读取、缓存创建和前端成本口径。

### 修复缓存命中率与缓存成本口径

- 后端 v2 聚合中的 `cached_tokens` 已表示缓存读取量；看板不再从中重复减去 `cache_write_tokens`，避免缓存创建量大于读取量时将命中率截断为 0
- Claude 等输入 token 不含缓存的上游，命中率分母统一包含非缓存输入、缓存读取和缓存创建；基于 `claude-opus-5` 最后一天 129 条真实记录复核，缓存读取 15,010,483、缓存创建 15,104,333，命中率由错误的 `0.0%` 修复为 `49.8%`
- `/dashboard-data` 兼容回退聚合统一让 `cached_tokens` 只保存缓存读取量，缓存创建继续由 `cache_write_tokens` 单独统计，避免正常摘要和兼容模式显示不同
- 前端本地成本回退按缓存读取价与缓存创建价分别计算；后端返回 `estimated_cost` 时仍直接采用后端结果

### 测试基准修复

- 协议兜底测试的公共夹具改用相对当前时间，避免固定的 2026-07-27 时间戳超出默认保留窗口后，记录被裁剪并导致 CI 全量测试失败
...
- `go vet ./...` 通过
- Dashboard 前端测试 141/141 通过
- `claude-opus-5` 实际导出数据复算缓存命中率为 `49.8%`
...
- 根因一:CPA 的 `conductor_execution.go` 先用 `ensureRequestedModelMetadata` **克隆**了 metadata map,再把 `selected_auth_id` 写进克隆体,而响应拦截器拿到的是 handler 手里的原 map,因此**所有**兜底记录都没有上游身份,一律按客户端协议猜成 `openai-compatible`
- 根因二:兜底与原生记录的配对通道 `isNativeProtocolCorrelationRecord` 限定 codex 家族,而 CPA 会把 `/v1/chat/completions`、`/v1/responses` 路由到任意上游(包括 Claude 协议上游代理的 deepseek 模型),Claude 上游的兜底记录因此永远配不上
- 根因三:配对的 token 比对逐字段进行,而 Claude 家族原生记录的 input 不含缓存、兜底解析的 OpenAI 形态响应体 input 含缓存,带缓存的请求天然对不上
- 根因四:完成时刻容差固定 1s,而漂移随请求时长累积(真实数据:730.3s 的流式请求差 1.028s、214.6s 的差 1.108s),长请求被判成两笔
- 修复:配对放开到任意上游家族;token 比对改用与约定无关的 `(总量 − 输出, 输出, 推理)`,缓存按各自口径折进总量;容差改为 `1s + 时长 × 0.5%`,上限 5s;端点一致性检查只在原生端点本身是 OpenAI 协议路径时生效(原生记录的 endpoint 是上游侧路径,`/v1/messages` 与客户端侧的 `/v1/chat/completions` 本就不是同一回事)
- 存量配对由双指针归并改为增广路最大匹配(Kuhn):窗口随时长变化后,双指针的"配不上就推进某一侧"会丢掉本该配上后一条 native 的记录;而单纯的"各挑最近的"贪心同样不保证最大匹配——F1 抢走 N1 后 F2 可能谁都够不着,尽管 F1→N2 / F2→N1 本来成立。候选按距离升序,因此在最大匹配的前提下仍优先配最近的一条
- 新增缓存命中判别项:关联键只比 (总量−输出, 输出, 推理),两笔缓存拆分不同但总量相同的请求会撞键。两侧都报非零命中时要求相等——CPA 的 Claude→OpenAI 翻译把 `cache_read_input_tokens` 原样写进 `prompt_tokens_details.cached_tokens`,口径一致可比。作为判别项而非键的一部分:兜底侧缺该字段时保持宽松,不会退回"配不上"。缓存**创建**量不可比(插件不解析 `cached_creation_tokens` 这个拼写,兜底侧恒为 0),故不参与判定
- 存量数据在重启、JSONL 重放、快照恢复和导入合并时按同一逻辑修复,无需开关
- 仍拿不到上游身份、也找不到可配对原生记录的兜底记录保持现状(记为 `openai-compatible`),不做推断

### 时段价格支持工作日/休息日

- 时段规则新增 `days` 星期维度(0=周日 … 6=周六),看板提供「每天 / 工作日 / 休息日」预设和逐日勾选,并在价格列表与价格查询卡片上显示适用日
- 重叠校验改为「星期相交 ∩ 时间相交」:星期不相交的规则可共用同一时间段,「工作日夜间半价 + 周末夜间原价」不再被判为冲突
- 跨午夜规则按请求实际落在的那一天判定:`22:00–06:00` 限定工作日时,周六凌晨不在规则内
- 向后兼容:规则不写 `days` 即表示每天,旧价格文件语义不变;覆盖整周的规则归一化回「每天」形态,存储与导出里「每天」只有一种表示
- 前后端校验对称,星期取值越界、重复或超过 7 个一律拒绝;非法 days 由 `serializedTimeRules` 原样带到校验,不再被顺手清洗后照常保存(此前越界项会被悄悄丢掉、全非法的集合塌成「每天」,而后端是拒绝的)
- 星期开关带本地化全名 `aria-label`(按当前语言从 `Intl` 取,不新增 i18n 条目),勾选后焦点回到同一个控件,键盘操作不再被重渲染打断
- 修复星期 chip 的样式被覆盖:同一块里的 `.timeRuleRow label{display:grid}` 特异度 (0,1,1) 高于裸类名 `.timeRuleDayChip` (0,1,0),chip 会被强制成 grid、复选框和星期文字上下堆叠。选择器改挂到 `.timeRuleRow` 下,并新增按特异度判定的回归测试(前端测试原本只断言 HTML,查不出这类问题)

### 汇率状态详情可达性

- 来源、更新时间和汇率错误此前只放在原生 `title` 里,而 `currencyStatus` 是不可聚焦的 `span`:触屏和键盘用户无法获取,`aria-live` 也只播报压缩后的正文
- 改为紧凑正文 + 可聚焦的详情开关(`aria-expanded` / `aria-controls` / 本地化 `aria-label`),`title` 保留不变
- 展开状态以按钮的 `aria-expanded` 为唯一真相来源,`index.html` 的初始属性只负责无 JS 时的形态;详情为空时强制收起,不留空展开区

### 验证
- `go vet ./...` 通过
- `go test -race -count=1 ./...` 通过
- Dashboard 前端测试 139/139 通过(新增 9 项)
- 用真实导出数据端到端复核 4 组配对:token 完全相同,完成时刻差 15ms / 1.8ms / 1.108s / 1.028s;总数 1212 → 1208,`openai-compatible` 分组清空

## v2.5.9 - 2026-08-23

本版本新增时段价格规则与 USD/CNY 金额切换，并修复 JSONL 重放时恢复记录的时间语义。

### 时段价格与汇率显示
- 手动模型价格支持最多 16 条不重叠的时段覆盖规则，按 `pricing_timezone` 和请求发生时间选择价格；未配置时段规则的旧价格文件保持全天价格语义
- 价格规则校验覆盖时间格式、跨午夜区间、重叠区间和模型作用域；保留的历史明细按时段价格计算成本差额，淘汰明细继续保留基础价格成本
- 看板金额支持 USD/CNY 切换；后端通过 HTTPS 获取 USD/CNY 汇率，并展示来源、更新时间及缓存、过期或回退状态
- 事件接口和 JSON 导出为每条记录提供 `cost_usd`，CSV 导出追加该字段，导出数据仍以 USD 为准

### 恢复与发布可靠性
- JSONL 重放恢复记录使用 synthetic timestamp，确保生产 replay 路径沿用记录自身的时间语义
- 删除误提交的运行时生成价格文件，并加入 `.gitignore`，避免测试价格覆盖正式模型计费

### 验证
- `go test -race -count=3 ./...` 通过
- `go vet ./...` 通过
- Dashboard 前端测试 130/130 通过
...
- 存量污染明细按签名(命中 == 创建 == X、缓存合计 == 2X、总量 == input+output+2X)还原:快照冷恢复后修复内存态,JSONL 重放与导入合并在入库口先还原再计算去重键;修复为**可选项**,由新增配置 `claude_cache_repair_enabled` 控制,默认关闭——旧数据中"真实命中恰好等于创建量"的合法记录与污染签名不可判定,请先用上游账单核对确属双计后再启用
- 持久化明细新增 `cache_read_tokens` 权威命中字段:修复版写入的记录携带该字段,历史修复据此免除误判,不会改写修复版之后的任何合法记录
- 重放与导入的去重键一律按归一化形态计算:同一请求的污染形态与修复形态收敛到同一个键,旧快照与当日分片、开关任意切换均不会双计,重复重启不累加;启用修复后导入明细以修复后形态写入持久化
...
### 上游接口详情费用
- “总花费”优先使用接口详情摘要中的服务端 `estimated_cost`，不再依赖浏览器额外加载模型价格表
- “模型分布”直接使用各 `model_stats` 条目的服务端估算费用，恢复每个模型的正确花费标注
- 旧版响应未提供 `estimated_cost` 时继续按 token 和本地模型价格计算，保持向后兼容

### 验证
- 增加未加载模型价格表时的服务端费用回归测试，覆盖接口总花费和模型分布花费
