...
本版本修复 CPA 原生用量记录缺少请求端点导致看板端点显示为 `-` 的问题，并补充跨持久化恢复与时间范围查询的回归覆盖。

- 提供 CPA v7.2.152 原生用量适配器补丁，将请求端点和流式标记传递给插件
- 保持响应拦截和流式响应拦截关闭，不恢复实时兜底记录
- 补丁覆盖 Responses、Chat Completions、Messages、取消请求、快照恢复和时间范围查询
- 通过插件 Go 测试、race 测试、`go vet`、共享库构建和 Dashboard 前端测试
