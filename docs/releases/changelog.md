...
- 修复匿名资源路由暴露统计、事件、价格、健康状态及导出文件的问题：只公开静态看板，所有数据请求统一经过 CPA 管理鉴权，并拒绝缓存的旧资源别名。旧资源数据 URL 改为管理 URL；刷新页面后继续使用原管理密钥。
- 完整用量备份新增独立后台任务，冻结账本块、计数与配置后逐条写出 v1 JSON，支持 gzip 和有校验的分块下载，不受事件筛选或条数限制；看板直接保存文件字节，旧后端回退保留原始 JSON 精度。
- 支持原生解压流的浏览器自动压缩传输完整备份，保持 256 KiB 分块上限与最终 JSON 格式；校验压缩数据及解压长度，旧浏览器使用未压缩分块。生成 CPU、下载时延和内存代价分别记录，不将压缩视为无成本优化。
- 分块和兼容信封的 base64 解码直接写入字节缓冲，避免字符串迭代产生中间数组；包含全部字节值、空输入和错误编码回归。
- 修复同一实例回导备份时清除客户端 hash 导致明细和账本重复计数；仅跳过已存在的完整指纹，保留跨实例导入规则及不同请求。
- 完整备份与事件任务共享活动/保留容量，按类型隔离状态和下载；备份等待统计锁可取消，文件校验和计算响应取消，失败编码清理半成品。
- 范围摘要与接口详情复用有预算的精确块汇总，边界记录仍逐条计算；追加、裁剪、修复、排序和改价正确失效，缓存满后的清理扫描每次查询最多一次。
- 修复同时间戳最近事件的选择，以及只排序、不淘汰时的块缓存失效；统计同频排序保持稳定。
- 导入响应直接读取一致的计数对，避免为两个标量展开和复制完整历史。
- 分块导出使用不受宿主 HTML 转义影响的版本标识，同时兼容旧客户端的 ETag；通过未修改 CPA 的真实 HTTP 与浏览器下载、重启及已同步增量的崩溃恢复检查。
- 查询冷/热代价、三版本组件对照和全部剩余发布门槛见[下一版本验收](next-version-validation.md)。
- 周期/关闭快照冻结归档分块后逐条写出，避免完整账本展开和整包 JSON 编码；并发修复、删除和过期采用块级复制，保持快照一致性。
- 账本身份字典减少重复大键，保留哈希碰撞时的精确比较与过期清理。
- SQLite 基础层升级 schema 3，补齐记录/状态/进度同事务提交、一致性备份与向新路径恢复；仍未启用运行时 SQLite 后端。
- 增加真实浏览器的筛选、翻页、网络重试及文件内容检查；阶段证据见[存储与浏览器验收](../plans/storage-stage-20260913/README.md)。

下一版本候选正在验收，版本号与正式 tag 尚未更新。与 v2.6.4 的同工具链对照及限制见[发布审查](../plans/release-readiness-20260912.md)。

- 明细达到上限后保留精简逐请求账本，维持范围统计、按请求时间重新计价及到期扣减；分块账本和明细底层空间复用降低写入分配，批量裁剪或过期后释放大量闲置明细容量。
- 增量事件索引、只读筛选、查询内价格复用和范围汇总计数优化，减少查询耗时；高基数客户端/模型优先使用小型 provider 计数数组。
- 后台导出冻结事件及成本，支持最多 256 KiB 分块下载、版本及 CRC32 校验、取消和资源回收；64 KiB 文件写缓冲减少小写入。看板协商 JSON 数组并直接保存文件字节，保留大整数精度、文件格式和旧后端兼容。
...
SQLite 尚未接管运行时历史存储。冻结导出、完整备份和有效历史仍有随数据量增长的内存开销；不得将本候选视为已达成整体 RSS/CPU 或百万条恒定内存目标。升级与回退须遵守[部署说明](../guides/cpa-usage.md#下一版本候选的升级与回退边界)。
