...
本版本修复了一处匿名数据泄露和一处升级重复入账缺陷，并交付范围查询优化、完整用量备份和更省分配的账本写入。**它不是"整体低资源"版本**：SQLite 仍未接管运行时历史存储，内存仍随有效历史增长，边界见文末。
...
- 修复前的反证：同一台未修改的 stock CPA 容器里，`v2.6.4` 写入 1200 条请求（明细上限 100，快照只留下 200 条逐请求记录），`v2.6.5` 恢复后累计变成 **2200**；修复后为 **1200**。升级后新增 40 条为 1240，再次重启仍为 1240。
...
- 降级不是无损的：本版本的快照新增了 `accounting` 语义，`v2.6.4` 无法回写。升级与回退步骤见[部署说明](../guides/cpa-usage.md#升级与回退边界)。
