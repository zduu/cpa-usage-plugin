...
元数据缓存最多保留 4096 个执行区间；完成后保留 30 分钟以接收延迟上报，匹配只接受最近 30 分钟开始的用量记录。容量耗尽或活动请求超过 30 分钟时，清空缓存并暂停补齐 30 分钟，避免淘汰候选后发生错误关联。用量计数和持久化仍正常工作。旧版 CPA 若不能可靠上报原生 usage，仍可能漏记，插件不恢复响应兜底。
