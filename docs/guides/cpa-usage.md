...
- **时段计费与货币显示**：手动价格可添加最多 16 条不重叠的 `[start,end)` 时段覆盖规则，按 `pricing_timezone` 和请求时间选价；价格查询面板会一并列出各时段的生效区间和覆盖价格（未覆盖的项回落到基础价格）。看板工具栏可切换 USD/CNY，后端成本与导出仍保持 USD，汇率状态会显示实时、缓存、过期或回退，并附带汇率单位、来源域名和更新时间；超过两倍刷新周期即判为过期，即使期间没有记录到拉取错误。过期、回退或持续拉取失败都会产生健康告警——回退状态下页面上的 CNY 是按配置的固定汇率换算的，并不反映真实汇率。`pricing_timezone` 填了非法时区时会保留原时区，并在运行状态和健康告警中给出原因。因 `max_details_per_model` 被淘汰的旧明细没有逐请求时间，重算时会保留其基础价格成本；仍保留的明细使用精确的时段覆盖价。导入、从 JSONL 冷恢复或从快照恢复时若记录本身没有时间戳，分桶和排序会补一个可用时间，但该记录会被标记为「时间戳非真实请求时间」并始终按基础价格计费，不会用服务器当时的钟点去套峰谷规则；该标记会随记录一同持久化，重启后仍然有效。后台导出任务在开始时会同时冻结时间上界和价格表，导出期间的改价或 models.dev 刷新不会让同一个文件前后页出现不同口径。
