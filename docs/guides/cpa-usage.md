...
`v2.2.8` 起的旧版本曾通过响应拦截延迟约 2 秒写入兜底记录。**当前源码已取消所有模型的实时兜底**，只注册原生 usage 和管理接口能力。新版 CPA 在用量适配器中通过 `context.WithoutCancel` 脱离请求取消状态，内置和插件执行器负责解析、发布用量；插件不再重复解析流式响应或维护实时匹配队列。历史兜底数据的导入、恢复及去重修复仍保留。

此次兼容性核对以本地 CPA 提交 `5208aec7` 为准，不能据此保证旧版 CPA 不漏记。升级插件前应同步使用包含上述修复的 CPA；不要将当前源码用于仍依赖响应兜底的旧主程序。详细依据和性能测试见 [资源使用审查](../issues/2026-09-06-resource-review.md)。
