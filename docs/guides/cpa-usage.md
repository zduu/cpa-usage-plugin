...
plugins:
  ...
  configs:
    usage-dashboard-zduu:
      ...
      # 时段计费按请求时间转换到该 IANA 时区，默认 Asia/Shanghai
      pricing_timezone: Asia/Shanghai
      # CNY 仅为看板显示层；后端获取汇率，失败时使用缓存或回退值
      exchange_rate_enabled: true
      exchange_rate_usd_cny_url: https://open.er-api.com/v6/latest/USD
      exchange_rate_refresh_interval_seconds: 21600
      exchange_rate_timeout_seconds: 5
      exchange_rate_fallback_usd_cny: 7.20
...
- **时段计费与货币显示**：手动价格可添加最多 16 条不重叠的 `[start,end)` 时段覆盖规则，按 `pricing_timezone` 和请求时间选价；价格查询面板会一并列出各时段的生效区间和覆盖价格（未覆盖的项回落到基础价格）。看板工具栏可切换 USD/CNY，后端成本与导出仍保持 USD，汇率状态会显示实时、缓存、过期或回退，并附带汇率单位、来源域名和更新时间；超过两倍刷新周期即判为过期，即使期间没有记录到拉取错误。过期、回退或持续拉取失败都会产生健康告警——回退状态下页面上的 CNY 是按配置的固定汇率换算的，并不反映真实汇率。`pricing_timezone` 填了非法时区时会保留原时区，并在运行状态和健康告警中给出原因。因 `max_details_per_model` 被淘汰的明细会转入精简统计账本，保留时间、用量、分组和历史去重所需字段，不保留请求头；保留窗口内的记录均使用精确的时段覆盖价。旧版本已经丢失逐请求信息的历史聚合仍只能按基础价格重算。导入、从 JSONL 冷恢复或从快照恢复时若记录本身没有时间戳，分桶和排序会补一个可用时间，但该记录会被标记为「时间戳非真实请求时间」并始终按基础价格计费，不会用服务器当时的钟点去套峰谷规则；该标记会随记录一同持久化，重启后仍然有效。后台导出任务在开始时会冻结匹配的事件值与成本，后续迟到上报、明细淘汰、改价或 models.dev 刷新不会改变文件内容。
- **事件成本字段**：事件接口、JSON/JSONL 导出（含后台分页导出）中的每条记录都带 `cost_usd`（后端按时段价格算好的 USD 成本，零价时段为显式的 `0`，看板不会再自行按基础价格重算）；CSV 导出把 `cost_usd` 追加在最后一列，不影响既有按列序解析的脚本。持久化的明细记录不写该字段，导入的数据也会在入库时清除它。
...
- 保留策略自动控制内存占用：`retention_days` 定义统计保留窗口，`max_details_per_model` 只限制每个上游接口/模型保留的最近请求明细数量。淘汰的明细保留精简统计账本，供时间范围统计、重算价格、去重与重启恢复使用；账本随 `retention_days` 到期清理。`retention_days: 0` 时账本会随请求量持续增长。完整统计导出包含 `accounting` 账本及其记录数；事件列表与事件导出只包含仍保留的完整明细。
...
`v2.2.8` 起的旧版本曾通过响应拦截延迟约 2 秒写入兜底记录。**当前源码已取消所有模型的实时兜底**，用量计数只接受原生 usage 上报，响应拦截和流式分片拦截仍关闭。历史兜底数据的导入、恢复及去重修复仍保留。

`v2.6.4` 的接口核对以 Go 模块代理发布的未修改 CPA `v7.2.152` 为准。插件通过已有的 `request.intercept_after` 获取 `Metadata.request_path`、已选凭据、执行模型和 `Stream`，通过 `request.complete` 关闭执行时间区间，在原生 `usage.handle` 写入前补齐缺失的元数据。无需修改 CPA 源码、应用补丁或重编译主程序。请求回调返回空修改对象，不改变请求头、请求体或响应，不从协议响应体解析 token。

旧宿主的原生 usage 不带请求 ID，因此这属于保守的元数据关联，不是逐请求 ID 的精确追踪。同一凭据、模型/别名和执行时间区间内的候选必须对端点、流式状态达成一致；并发候选冲突时保持未知。仅接受宿主实际提供的请求路径，不根据上游或模型猜测；不覆盖原生已有的端点和显式流式字段。未提供请求回调的 SDK 调用、缺少必要凭据/时间信息的记录、已过保留期的请求无法保证补齐，历史空端点也不会被补造。

元数据缓存最多保留 4096 个执行区间；完成后保留 30 分钟以接收延迟上报，匹配只接受最近 30 分钟开始的用量记录。容量耗尽或活动请求超过 30 分钟时，清空缓存并暂停补齐 30 分钟，避免淘汰候选后发生错误关联。用量计数和持久化仍正常工作。旧版 CPA 若不能可靠上报原生 usage，仍可能漏记，插件不恢复响应兜底。`v2.6.3` 的历史资源审查见 [资源使用审查](../issues/2026-09-06-resource-review.md)。

需要注意的边界（以下兜底字段说明只适用于旧记录）：
...
开发中候选的周期与关闭快照：归档账本按不可变分块冻结，再逐条编码写入临时文件；可见明细和聚合仍复制。并发修复或过期通过块级复制保持冻结数据，完成 flush/fsync 后原子替换快照，编码或写入失败保留旧文件。格式及现有刷新、同步、快照间隔配置不变；启动读取完整快照的内存开销尚未消除。
