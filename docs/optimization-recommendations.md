...
- 看板导出按钮默认使用后台导出任务：创建任务后轮询状态，完成后下载临时文件结果，并主动删除任务；后台任务限制 2 个活跃任务、16 个保留任务和约 15 分钟保留窗口。
...
当前已支持 `format=csv|jsonl`、`gzip=1`、`export_max_records` 和单次请求 `limit`，并且看板导出默认走后台任务，减少浏览器侧大数组转换开销和单个管理请求的长时间占用。但 CPA 插件管理响应当前仍是单个 `ManagementResponse.Body []byte`，没有真正的 chunked streaming 能力；最终下载仍会通过一次性响应体返回受上限保护的结果。
