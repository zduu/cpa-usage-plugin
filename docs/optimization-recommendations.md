...
- 看板导出按钮默认使用后台导出任务：创建任务后轮询状态，生成阶段按页扫描并写入临时文件，不构造完整事件数组；完成后下载临时文件结果并主动删除任务。后台任务限制 2 个活跃任务、16 个保留任务和约 15 分钟保留窗口。
...
当前已支持 `format=csv|jsonl`、`gzip=1`、`export_max_records` 和单次请求 `limit`，并且看板导出默认走后台任务，生成阶段按页扫描并写入临时文件，减少浏览器侧大数组转换开销、完整事件数组分配和单个管理请求的长时间占用。但 CPA 插件管理响应当前仍是单个 `ManagementResponse.Body []byte`，没有真正的 chunked streaming 能力；最终下载仍会通过一次性响应体返回受上限保护的结果。

数据量继续上来后建议：

- 推动管理接口协议支持 chunked streaming。
- 为大导出增加强制时间窗口。
...
20. 看板导出切换为后台导出任务，生成阶段不再长时间占用单个管理请求，并通过任务并发/保留上限控制大导出压力。
21. 后台导出生成阶段按页扫描筛选结果并写入临时文件，不再构造完整事件数组，进一步降低大导出内存峰值。

下一步建议：

1. 生产配置默认开启持久化，文档强调 volume、flush、retention 和升级前导出。
2. 推动管理接口支持真流式导出，进一步降低超大导出的内存峰值。
